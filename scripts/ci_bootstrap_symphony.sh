...
rm -f /tmp/symphony.env /tmp/codex-auth.json /tmp/symphony-github-key
...
sudo -u symphony -H bash -lc 'export CODEX_HOME=/var/lib/symphony/.codex; export PATH=/var/lib/symphony/.local/bin:/var/lib/symphony/.local/share/mise/shims:$PATH; codex login status'
sudo -u symphony -H bash -lc 'set -a; . /etc/symphony/env; set +a; gh api user >/dev/null'
sudo -u symphony -H bash -lc 'rm -rf /tmp/agent-flow-bootstrap-smoke; git clone --depth 1 git@github.com:AttentionWorld/agent-flow.git /tmp/agent-flow-bootstrap-smoke >/dev/null; test -d /tmp/agent-flow-bootstrap-smoke/.git; rm -rf /tmp/agent-flow-bootstrap-smoke'
